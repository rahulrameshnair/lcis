...
'''
Documentation
1. print_lci_all is used to output all the activities and the corresponding exchanges from a selected brightway database. The printing is done in dictionary format with all the details such as the linked database names, activity codes and so on.

2. print_lci_classify is used to classify and count those exchanges and activities inherent/intrinsic to the database and those that copied/imported from other databases. This is because, in BW, copied activities have their exchanges stored in a dictionary format while intrinsic activities have their exchanges stored in dictionary form. Within each such activities, the number of technosphere flows and biosphere flows are to be identified.
    There is another type of flow called "downstream consumers" in the activity browser. This just means that if the product from one of the activity (a1, say) is used as a technosphere input for the activity (a2, say). Then, a2 becomes the downstream consumer of a1. This is not classified here because it is a just an interdependency.

'''
def print_lci_all(selected_database):
...
def print_lci_classify(selected_database):
    ...
    for activity in selected_database:
        ...
        if 'worksheet name' not in activity:
            ...
            print ('\t', str(total_activity_counter)+'.',  activity, "Type: Intrinsic", " with", technosphere_counter, " technosphere flows", biosphere_counter, " biosphere flows", product_counter, " products",  '\n')
            print('\t \t', 'This activity', activity['name'], 'relies on the databases:', name_database_dependecies, '\n')
        else:
