...
This helper depends on Brightway2 inside the function body. It is useful when
an activity exchange stores only a database/code tuple and the user needs the
human-readable exchange name, unit, and location.
...
def invoke_exchanges(database_name, exchange_key):
    ...
    """
    """
    The function opens the requested Brightway database, resolves the exchange
    key, and returns the three display fields most often needed when checking
    imported activities by hand.
    """
