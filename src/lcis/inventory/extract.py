...
This file supports LCIS inventory export by resolving Brightway exchange
references into fuller activity/flow dictionaries. It depends on Brightway2
inside the methods that access databases and is used by inventory extraction
code when exchange records only contain database/code tuples.
"""


class Extract_from_lci:
    """Helpers for resolving and reshaping exchange details from Brightway."""
    """
    Brightway exchange records often point to another database object through
    an ``input`` or ``output`` tuple. This class centralizes the lookup and
    reshaping logic so LCIS inventory records can include readable exchange
    details rather than only internal Brightway identifiers.
    """
    ...
    def extract_details(self, name_selected_database, exchange_name, exchange_type):
        ...
        """
        """
        This method performs a targeted Brightway lookup and returns only the
        fields needed by older exchange-display workflows. Biosphere exchanges
        do not have reference products/locations in the same shape as
        technosphere or production exchanges, so their category tuple is
        flattened into a readable string.
        """
    ...
    def extract_all_details(self, name_selected_database, exchange_name):
        ...
        """
        """
        This is the broader lookup used when LCIS needs the full Brightway
        object dictionary. The caller can then select schema-relevant keys
        without losing access to fields that may matter for research review.
        """
    ...
    def organize_all_details(
        ...
        """
        """
        This function augments an existing details dictionary with selected
        values from the activity exchange record. For biosphere flows it also
        converts the category tuple into the LCIS-friendly ``primary::detail``
        text format used elsewhere in the inventory export.
        """
    ...
    def classify_exchanges(self, exchange_dict, exchange_type):
        ...
        """
        """
        Brightway stores the relevant lookup tuple on different sides of the
        exchange depending on flow type. Technosphere and biosphere exchanges
        point to their source through ``input``; production exchanges use the
        ``output`` tuple for the produced activity.
        """
    ...
    def update_exchange_details(self, exchange_details, specified_keys):
        ...
        """
        """
        The LCIS export does not always need every Brightway field. This helper
        creates a smaller dictionary containing only the keys requested by the
        caller, keeping exported exchange records focused on the active schema.
        """
