"""Build LCIS inventory data structures from a Brightway database.

This module extracts project, database, activity, and exchange information from
Brightway and prepares it for LCIS export. It depends on NetworkX only for the
legacy network helper, and on :mod:`lcis.inventory.extract` for resolving
Brightway exchange references into full exchange details.
"""

import networkx as nx

from .extract import Extract_from_lci


class LCI:
    """Container for extracting LCIS-style inventory data from Brightway."""
    """
    The class holds the selected Brightway database and exposes methods that
    turn Brightway's activity/exchange objects into plain Python dictionaries.
    Those dictionaries are easier to export, inspect, and validate in a
    research workflow than live Brightway objects.
    """
    ...
    def __init__(self, project_name, database_name) -> None:
        ...
        """
        """
        Brightway project selection is global state, so the constructor sets
        the current project before opening the database. The extracted LCIS
        list is initialized empty and later populated by ``database_list``.
        """
    ...
    def database_list(self, keys_biosphere, keys_technosphere, keys_production):
        ...
        """
        """
        This is the main inventory extraction routine. It first records
        Brightway parameters, then walks every activity and rewrites exchange
        references into richer dictionaries using ``Extract_from_lci``. The
        final structure keeps parameters in position 0 and activities in
        position 1 for compatibility with the existing LCIS export workflow.
        """
    ...
    def key_search(node_attributes):
        ...
        """
        """
        Network and serialization tools can struggle with values such as
        tuples, objects, or other complex types. This helper scans nested node
        attributes and reports which keys contain non-primitive values so the
        caller can remove or convert them before export or visualization.
        """
    ...
    def recursive_search(node_details):
        ...
        """
        """
        This function normalizes nested node dictionaries in place. It turns
        category tuples into readable ``::`` strings and replaces ``None`` with
        a literal marker, which makes downstream graph attributes easier to
    ...
    def network(lci_database):
        ...
        for activity in activities_dict:
            ...
            node_attributes[activity_name] = activity_info
