...
This module contains console inspection helpers for Brightway databases. It has
no top-level third-party imports because callers pass in an already selected
...
def print_all(selected_database):
    ...
    """
    """
    This is a raw inspection view. It prints Brightway's own dictionaries for
    each activity and exchange so a researcher can see exactly what the source
    database contains before LCIS reshapes or exports the data.
    """
...
def print_classified(selected_database):
    ...
    """
    """
    This function summarizes a database in a more interpretive way than
    ``print_all``. It separates intrinsic activities from imported activities
    using the ``worksheet name`` marker and counts exchange types so the user
    can quickly understand the composition of the selected database.
    """
